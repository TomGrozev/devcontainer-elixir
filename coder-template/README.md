...
| `coder_app.ompweb`                        | Proxies the ompweb browser UI for omp via subdomain (`http://localhost:30177`). Healthy only while the ompweb daemon started by the Start ompweb button is running. Healthcheck at `/api/home`.                                                             |
| `coder_app.start_ompweb`                  | Button: launches the ompweb server via `npx -y @kahme247/ompweb` (npm cache lives on the PVC, so it downloads once and stays warm) as a standalone daemon (not a miao pooled session); ompweb's per-session omp agents are spawned through captain-miao (`miao launch omp`). Idempotent — one ompweb at a time. |
...
- **omp** — the **Start ompweb** button starts the **ompweb** browser UI for the omp agent as a standalone daemon on `:30177` (it is a plain HTTP server, not a pty session, so it does not live in the miao pool). Each chat session's omp agent is spawned **through captain-miao** (`miao launch omp` via the `omp-web-omp-bin` shim), so those agents get hooks and surface in the miao dashboard. Open the **ompweb** app to browse sessions and drive new ones. **Start opencode** and **OpenCode** are the opencode equivalents.
...
The buttons are idempotent and exit early when their health endpoint is reachable. If that's wrong (the process is dead but the port is still held), stop it and start fresh. For **ompweb**, kill its daemon (e.g. `pkill -f kahme247/ompweb`); its omp agents live in the miao pool and can be stopped from the miao dashboard. For **opencode**, stop the pooled session from the miao dashboard (`x`), or run `miao-server daemon stop` to tear the pool down and start fresh.
